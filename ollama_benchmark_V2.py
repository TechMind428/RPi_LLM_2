...
def find_ollama_proc() -> Optional[psutil.Process]:
    ...
    if not candidates:
        return None
    # prefer the one currently consuming more CPU
    best = candidates[0]
    best_cpu = -1.0
    for p in candidates:
        try:
...
# =========================
# Streaming run via API (curl) with added metrics
# =========================

def run_prompt_via_api(model: str, prompt: str) -> Dict[str, Any]:
    # Use curl to read streaming JSON (line-delimited)
    ...
    # Target process for resource sampling
    ...
    eval_count = None         # from API (token count)
    eval_duration_ns = None   # from API (nanoseconds)
    ...
    for line in iter(proc.stdout.readline, ''):
        ...
        if not s:
            ...
            # Added: iowait + swap_max sampling
            try:
        ...
        # streaming done record may include token stats
        if data.get("done", False):
            # metrics keys vary by version; try common names
            eval_count = data.get("eval_count", eval_count)
            eval_duration_ns = data.get("eval_duration", eval_duration_ns)
            # Sometimes total_duration, prompt_eval_* also exist
            break

        # sample while streaming
        cpu, mem = sample_resources(ollama_proc)
        cpu_samples.append(cpu)
        mem_samples.append(mem)
        t = get_cpu_temp()
...
# =========================
# One run wrapper (adds original columns)
# =========================
...
def run_prompt(model: str, prompt: str, phase: str, run_id: int, hwinfo: Dict[str,Any]) -> Dict[str,Any]:
    ...
    # Add all disk fields from api_result
...
def main():
    parser = argparse.ArgumentParser(description="Ollama benchmark with swap/disk/iowait metrics (V2).")
    ...
    if args.output:
        ...
        outfile = f"ollama_benchmark_V2_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.csv"

    # CSV writer with dynamic fieldnames based on the first row
